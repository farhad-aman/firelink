...
def drop_source_torrent(client, status: dict) -> bool:
    """Remove the .torrent that was fetched to start this download."""
...
def final_path(path: Path, cfg: Config, url: str, status: dict) -> Path:
    """Where this download ends up.

    Anything from a torrent stays where it downloaded: it was sent to the
    torrents folder on purpose, before anyone knew what it held, and moving it
    by content afterwards would undo that.
    """
    if torrent.is_torrent_status(status):
        return path
    return relocate(path, cfg, url)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if mode == "complete" and record["path"]:
            ...
            final = final_path(original, cfg, record["url"], status)
